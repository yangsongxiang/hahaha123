...
class TransDist:
    def __init__(
        ...
        r_min=0, r_max=2985,
        Nr=200,
        vr_min=-369.1406, vr_max=369.1406,
        Nv=64,
        T=1,
        sigma_r=0.00001,
        sigma_v=0.00001,
        ...
        # Create linearly spaced vectors of range and velocity bin centers
        self.range_vec = torch.linspace(r_min, r_max, Nr)
        self.vr_vec    = torch.linspace(vr_min, vr_max, Nv)
        # Half-bin widths (physical)
        self.r_bin     = float((self.range_vec[1] - self.range_vec[0]) / 2)
        self.v_bin     = float((self.vr_vec[1]    - self.vr_vec[0])    / 2)
        self.T         = T      # time between frames
        self.Nr        = Nr
        self.Nv        = Nv
        self.RW        = RW

        # self.prob[dr, dv] will store log P(¦¤r = dr bins, ¦¤v = dv bins)
        self.prob = torch.zeros([Nr, Nv], dtype=torch.float32)

        if RW:
            # -------- Random-walk with acceleration noise --------
            self.sigma_a = sigma_a
            dist_a = dist.Normal(0, sigma_a)
            r_const = (T**2) / 2
            for dr in range(Nr):
                for dv in range(Nv):
                    # convert dr,dv to acceleration bounds
                    r_max = (2*dr + 1) * self.r_bin / r_const
                    r_min = (2*dr - 1) * self.r_bin / r_const
                    v_max = (2*dv + 1) * self.v_bin /     T
                    v_min = (2*dv - 1) * self.v_bin /     T
                    a_upper = min(r_max, v_max)
                    a_lower = max(r_min, v_min)
                    if a_lower > a_upper:
                        self.prob[dr, dv] = -torch.inf
                    else:
                        p = dist_a.cdf(a_upper) - dist_a.cdf(a_lower)
                        self.prob[dr, dv] = torch.log(p + 1e-12)
            self.dist_a = dist_a

        else:
            # -------- Constant-velocity + process noise --------
            # Effective process noise on range and velocity
            sigma_r_eff = torch.sqrt(
                torch.tensor((sigma_a*(T**2)/2)**2 + sigma_r**2,
                             dtype=torch.float32)
            )
            sigma_v_eff = torch.sqrt(
                torch.tensor((sigma_a*T)**2 + sigma_v**2,
                             dtype=torch.float32)
            )
            dist_r = dist.Normal(0, sigma_r_eff)
            dist_v = dist.Normal(0, sigma_v_eff)
            self.dist_r = dist_r
            self.dist_v = dist_v

            # Build log-prob table for dr = 0¡­Nr-1
            # Physical offset = dr * full_bin_width ¡À half_bin
            full_r_width = 2 * self.r_bin
            for dr in range(Nr):
                upper = dr*full_r_width + self.r_bin
                lower = max(dr*full_r_width - self.r_bin, 0.0)
                # CDF inputs must be Tensors
                up_t   = torch.tensor(upper, dtype=torch.float32)
                low_t  = torch.tensor(lower, dtype=torch.float32)
                p_r    = dist_r.cdf(up_t) - dist_r.cdf(low_t)
                self.prob[dr, :] = torch.log(p_r + 1e-12)

            # Build log-prob table for dv = 0¡­Nv-1
            full_v_width = 2 * self.v_bin
            for dv in range(Nv):
                upper = dv*full_v_width + self.v_bin
                lower = max(dv*full_v_width - self.v_bin, 0.0)
                up_t   = torch.tensor(upper, dtype=torch.float32)
                low_t  = torch.tensor(lower, dtype=torch.float32)
                p_v    = dist_v.cdf(up_t) - dist_v.cdf(low_t)
                # add velocity log-prob to each dr row
                self.prob[:, dv] += torch.log(p_v + 1e-12)

        # After building self.prob[dr,dv], expand into full S¡ÁS transition matrix
        self._build_full_transition_matrix()

    def _build_full_transition_matrix(self):
        """
        Expand the dr,dv log-prob table into a dense S¡ÁS matrix P,
        where S = Nr*Nv and P[i,j] = P(state_i -> state_j).
        """
        S = self.Nr * self.Nv
        P = np.zeros((S, S), dtype=np.float32)
        for i in range(S):
            ri, vi = divmod(i, self.Nv)
            for j in range(S):
                rj, vj = divmod(j, self.Nv)
                dr = abs(ri - rj)
                dv = abs(vi - vj)
                # exponentiate log-prob to get linear prob
                P[i, j] = float(torch.exp(self.prob[dr, dv]).item())
        # normalize each row to sum to 1
        P /= np.maximum(P.sum(axis=1, keepdims=True), 1e-12)
        self.P = P

    def get_transition_matrix(self):
        """Return the S¡ÁS transition probability matrix."""
        return self.P

    def ind2val(self, r, v):
        """Convert discrete bin indices to physical (range, velocity)."""
        return float(self.range_vec[r]), float(self.vr_vec[v])

    def next(self, r, v):
        """Predict next physical (r,v) by constant velocity."""
        return r + v*self.T, v

    def prev(self, r, v):
        """Invert one step of constant-velocity motion to get previous (r,v) bins."""
        v_prev = self.vr_vec[v]
        r_prev = self.range_vec[r] - v_prev * self.T
        return self.val2idx(r_prev, v_prev)

    def val2idx(self, r_val, v_val):
        """Find nearest bin indices for physical (r_val, v_val)."""
        r_idx = int(torch.argmin(torch.abs(self.range_vec - r_val)))
        v_idx = int(torch.argmin(torch.abs(self.vr_vec    - v_val)))
        return r_idx, v_idx

    # (Other beam-search helpers unchanged¡­)
...
# (beam_mask, masked_argmax, get_top_n_mask, Find_Accuracy remain unchanged)
